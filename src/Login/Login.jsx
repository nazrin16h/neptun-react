...
import { login as postApiLogin } from '../services/api'
...
function Login() {
    ...
    function loginOl() {
        const data = postApiLogin(obj).then(res => {
            ...
            <Link to="/login">
                <div className="min-h-screen bg-gray-100 p-8 flex flex-col lg:flex-row gap-6">
                    ...
                    <div className="bg-white p-6 rounded-lg shadow-md flex-1">
                        ...
                        <input  value={obj.login} onChange={(e) => { setObj({ ...obj, login: e.target.value }) }} type="email" className="w-full mb-4 p-2 runded bg-blue-100 rounded-full" />
                        ...
                        <button onClick={loginOl} className="bg-orange-500 ml-3 hover:bg-orange-600 text-white rounded-full font-semibold py-2 px-6 w-full sm:w-auto">
                            Giriş
                        </button>
                    </div>
